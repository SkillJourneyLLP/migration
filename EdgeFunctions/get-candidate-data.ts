...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: interviewData, error: interviewError } = await supabase.from('interview').select('*').eq('id', candidateData.interview_id).single();
    if (interviewError) {
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        ...
        interviewId: candidateData.interview_id,
        openingStatement: interviewData.opening_statement,
