...
async function callElevenLabsWithRetry(formData, apiKey, retries = MAX_RETRIES) {
...
serve(async (req)=>{
  ...
  try {
    ...
    // Call ElevenLabs with retry logic using the dynamic API key
    const response = await callElevenLabsWithRetry(elevenLabsFormData, Deno.env.get(`ELEVENLABS_API_KEY_${apikeyIndex}`));
    if (!response.ok) {
      const errorText = await response.text();
    ...
    console.error('Error in speech-to-text-dynamic function:', error);
    return new Response(JSON.stringify({
