...
async function generateResumeSummary(supabaseClient, pdfBase64) {
  try {
    const { data: resumeData, error: resumeError } = await supabaseClient.functions.invoke('resume-summary', {
      body: {
        pdfBase64: pdfBase64
      }
    });
...
const handler = async (req)=>{
  ...
  try {
    ...
    const { data: interviewData, error: interviewError } = await supabaseClient.from('interview').select('*, users!inner(organization_id)').eq('id', interviewId).single();
    ...
    const { data: orgData } = await supabaseClient.from('organization').select('name').eq('id', interviewData.users.organization_id).single();
    ...
    for (const candidateData of candidates){
      try {
        ...
        if (candidateData.email) {
          const { error: emailError } = await supabaseClient.functions.invoke('send-interview-confirmation', {
            body: {
              candidateEmail: candidateData.email,
              candidateName: candidateData.name,
              interviewLink: interviewLink,
              position: interviewData.position,
              startDate: interviewData.start_date,
              startTime: interviewData.start_time,
              endDate: interviewData.end_date,
